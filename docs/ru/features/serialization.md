...
W.Serializer.SetSnapshotHandler(
    ...
    version: 0,                                   // Версия
    (ref MemoryPackWriter writer, SnapshotWriteParams param) => {            // Писатель кастомных данных
        writer.WriteDateTime(DateTime.Now);
        Console.WriteLine("Saved current time");
    },
    (ref MemoryPackReader reader, ushort version, SnapshotReadParams param) => { // Читатель кастомных данных
